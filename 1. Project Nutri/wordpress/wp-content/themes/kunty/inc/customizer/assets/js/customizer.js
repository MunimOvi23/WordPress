jQuery( document ).ready(function($) {
	...
	jQuery(document).ready(function ($) {
		...
		$('body').on('click', '#sub-accordion-panel-kunty_frontpage_settings .control-subsection .accordion-section-title', function(event) {
	...
	function scrollToSection( section_id ){
		...
		var preview_section_id = "hero-section";

		var $contents = jQuery('#customize-preview iframe').contents();

		switch ( section_id ) {
			
			case 'accordion-section-about_section_settings':
			preview_section_id = "intro-section";
			break;

			case 'accordion-section-services_section_settings':
			preview_section_id = "service-section";
			break;
			
			case 'accordion-section-funfacts_section_settings':
			preview_section_id = "funfacts-section";
			break;

			case 'accordion-section-portfolio_section_settings':
			preview_section_id = "portfolio-section";
			break;

			case 'accordion-section-feedback_section_settings':
			preview_section_id = "feedback-section";
			break;
			
			case 'accordion-section-team_section_settings':
			preview_section_id = "team-section";
			break;

			case 'accordion-section-cta_section_settings':
			preview_section_id = "cta-section";
			break;
			
			case 'accordion-section-pricing_section_settings':
			preview_section_id = "pricing-section";
			break;

			case 'accordion-section-news_section_settings':
			preview_section_id = "news-section";
			break;
			
			case 'accordion-section-brands_section_settings':
			preview_section_id = "brands-section";
			break;
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					kuntyAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				kuntyGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			kuntyGetAllInputs($(this).parent().parent().parent());
		}
	});
	...
	function kuntyAppendRow($element, defaultValue = '') {
	...
	function kuntyGetAllInputs($element) {
	...
	$('.multi-image-checkbox').on('change', function () {
	  kuntyGetAllImageCheckboxes($(this).parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function kuntyGetAllImageCheckboxes($element) {
	...
	$( ".pill_checkbox_control .sortable" ).sortable({
		...
		update: function( event, ui ) {
			kuntyGetAllPillCheckboxes($(this).parent());
		}
	});

	$('.pill_checkbox_control .sortable-pill-checkbox').on('change', function () {
		kuntyGetAllPillCheckboxes($(this).parent().parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function kuntyGetAllPillCheckboxes($element) {
	...
	$('.google-fonts-list').on('change', function() {
		...
		var indexes = $.map(bodyfontcontrol.kuntyfontslist, function(obj, index) {
		...
		$.each(bodyfontcontrol.kuntyfontslist[index].variants, function(val, text) {
		...
		$(this).parent().parent().find('.google-fonts-category').val(bodyfontcontrol.kuntyfontslist[index].category);

		kuntyGetAllSelects($(this).parent().parent());
	});
	...
	function kuntyGetAllSelects($element) {
		var selectedFont = {
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].kuntytinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].kuntytinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].kuntymediabuttons;
 ...
 */
( function( $, api ) {
	api.sectionConstructor['kunty-upsell'] = api.Section.extend( {
